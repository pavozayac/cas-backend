...
async def update_profile_avatar(file: UploadFile = Form(...), db: Session = Depends(get_database), profile: models.Profile = Depends(LoginAuth)):
    avatar_in = schemas.AvatarIn(filename=file.filename)
...
async def delete_profile_avatar(db: Session = Depends(get_database), profile: models.Profile = Depends(LoginAuth)):
